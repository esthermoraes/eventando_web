...
if (autenticar($db_con)) {
    ...
    if (isset($_GET['limit']) && isset($_GET['offset']) && isset($_GET['user_email'])) {
        ...
        if ($consultaUsuario->rowCount() > 0) {
            ...
            // Consulta SQL para obter os eventos favoritos usando uma consulta preparada
            $consultaEventos = $db_con->prepare("
                SELECT
                    e.id_evento,
                    e.nome,
                    e.data_prevista,
            ...
            $consultaEventos->bindParam(':user_id', $user_id, PDO::PARAM_INT);
